...
def do_marking_run(overall_marking_percentage, experiment_directory, tensorboard_log_directory, augment=True):
    ...
    marking_network = resnet50(num_classes=100)
    checkpoint_path = "experiments/resnet50.pth"
...
def do_marking_run_multiclass(overall_marking_percentage, experiment_directory, tensorboard_log_directory, augment=True):
    ...
    # Carriers
    marking_network_fc_feature_size = 2048
    carriers = torch.randn(len(training_set.classes), marking_network_fc_feature_size)
    carriers /= torch.norm(carriers, dim=1, keepdim=True)
    torch.save(carriers, os.path.join(experiment_directory, "carriers.pth"))
...
def do_training_run(run_name, augment):
    ...
    tensorboard_log_directory = f"runs/table2_mark_resnet50_{run_name}_target"
    epochs = 200
    output_directory = f"experiments/cifar100/table2_mark_resnet50/{run_name}/marked_classifier"
    marked_images_directory = f"experiments/cifar100/table2_mark_resnet50/{run_name}/marked_images"
...
def calculate_p_values(marking_percentages):
    logfile_path = f"experiments/cifar100/table2_mark_resnet50/detect_radioactivity.log"
    ...
    # marking_network = resnet(num_classes=100, depth=164, block_name='bottleneck')
    marking_network = resnet50(num_classes=100)
    marking_checkpoint_path = "experiments/resnet50.pth"
    ...
    for run in marking_percentages:
        ...
        carrier_path = f"experiments/cifar100/table2_mark_resnet50/{run_name}/carriers.pth"
        ...
        target_checkpoint_path = f"experiments/cifar100/table2_mark_resnet50/{run_name}/marked_classifier/checkpoint.pth"
...
def generate_table_2(marking_percentages, p_values):
    ...
    for run in marking_percentages:
        run_name = f"{run}_percent"
        marked_checkpoint_path = f"experiments/cifar100/table2_mark_resnet50/{run_name}/marked_classifier/checkpoint.pth"
    ...
    table = ax.table(cellText=cell_text,
                     ...
                     loc='center')
    plt.savefig("experiments/cifar100/table2_mark_resnet50/table2.png")
    plt.show()


if __name__ == '__main__':
    marking_percentages = [2, 5, 10, 20]
    p_values_file = "experiments/cifar100/table2_mark_resnet50/p_values.pth"
    ...
    if local_rank == 0:
        for marking_percentage in marking_percentages:
            experiment_directory = os.path.join("experiments/cifar100/table2_mark_resnet50", f"{marking_percentage}_percent")
            if os.path.exists(os.path.join(experiment_directory, "marking.complete")):
                ...
                #     continue
                continue
            tensorboard_log_directory = os.path.join("runs", "table2_mark_resnet50", f"{marking_percentage}_percent", "marking")
    ...
    # for marking_percentage in marking_percentages:
    for marking_percentage in [2, 20]:
